...
def reqres_api(method, url, **kwargs):
    ...
    method = method.upper()
    with allure.step(
            f'Отправляем {method} запрос на reqres.in{url}. Параметры: {args if len(args) != 0 else "отсутствуют"}'):
...
@allure.title('Get status code 200')
...
@allure.label('owner', 'Elena Rogozina')
@allure.feature('reqres')
@allure.title('GET user is not found')
