...
def catfact_api(method, url, **kwargs):
    ...
    method = method.upper()
    with allure.step(
            f'Отправляем {method} запрос на catfact.ninja{url}. Параметры: {args if len(args) != 0 else "отсутствуют"}'):
...
@allure.feature('catfact')
@allure.title('Get breeds')
def test_get_breeds():
    page = 1
...
@allure.label('owner', 'Elena Rogozina')
@allure.feature('catfact')
@allure.title('Schema validation')
